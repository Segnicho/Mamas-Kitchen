...
const Reserve = () => {
  ...
  useEffect(() => {
      const fetchAfood = async () => {
          ...
          const response = await axios.get(`${baseUrl}/rooms/${userId}`)
...
return (
  ...
  { reservedRooms && reservedRooms.map((order, indx) => (
      <div
      key={indx}
      data-aos="fade-up"
  ...
  {!reservedRooms && <div className='h-screen flex justify-center items-center'>
      <h1 className='text-3xl'>you have no reserved rooms</h1>
      </div>}
</div>
